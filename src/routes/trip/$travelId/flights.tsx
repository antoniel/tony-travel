    ...
    <div className="space-y-4">
      {flightsWithoutParticipants > 0 && (
        <Alert>
          ...
          <AlertDescription>
            {flightsWithoutParticipants} voo(s) sem participantes. Adicione pessoas aos voos para melhor organização.
          </AlertDescription>
        </Alert>
      )}
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6">
      <div className="space-y-3">
        ...
        <p className="text-lg text-muted-foreground">Gerencie os voos de todos os membros da viagem</p>
      </div>
      {canWrite ? (
        <Dialog open={isAddFlightOpen} onOpenChange={setIsAddFlightOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <AddOrEditFlightForm travelId={travelId} members={members} onClose={() => setIsAddFlightOpen(false)} />
          </DialogContent>
        </Dialog>
      ) : null}
    </div>
...
function FlightsPage() {
  ...
  const flightsQuery = useQuery(orpc.flightRoutes.getFlightsByTravel.queryOptions({ input: { travelId } }))
  ...
  const deleteFlightMutation = useMutation(orpc.flightRoutes.deleteFlight.mutationOptions())
  ...
  const flightsWithoutParticipants = allFlights.filter((f) => f.participants.length === 0)
    ...
    <Card className="relative overflow-hidden border-dashed border-2 border-border/50 bg-gradient-to-br from-background via-muted/20 to-background">
      ...
      <CardContent className="relative p-16 text-center">
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <h3 className="text-2xl font-bold text-foreground tracking-tight">Nenhum voo cadastrado</h3>
            <p className="text-lg text-muted-foreground max-w-md mx-auto">
              Comece adicionando os primeiros voos da sua viagem para organizar todos os deslocamentos
            </p>
          </div>
...
function FlightGroupHeader({ group, isOpen, onToggle }: { group: FlightGroup; isOpen: boolean; onToggle: () => void }) {
  const totalPassengers = group.flights.reduce((total, flight) => total + flight.participants.length, 0)
      ...
      <CollapsibleTrigger asChild>
        ...
        >
          ...
          <div className="flex-1 text-left">
            <h2 className="text-xl font-bold text-foreground tracking-tight">Partindo de {group.originAirport}</h2>
            <p className="text-sm text-muted-foreground font-medium">
              {group.flights.length} {group.flights.length === 1 ? "voo" : "voos"} • {totalPassengers}{" "}
          ...
          <div className="flex items-center gap-2">
            <div className="px-3 py-1 rounded-full bg-background/80 border border-border/50">
              <span className="text-xs font-semibold text-primary">{group.flights.length} voos</span>
            </div>
            {isOpen ? (
    ...
    setOpenGroups((prevOpenGroups) => {
      const initialState = flightGroups.reduce(
        (acc, group) => {
          ...
          acc[group.originAirport] = prevOpenGroups[group.originAirport] ?? true
          return acc
        },
    ...
    <div className="space-y-8">
      {flightGroups.map((group) => (
        <Collapsible
          ...
          onOpenChange={(open) => setOpenGroups((prev) => ({ ...prev, [group.originAirport]: open }))}
        >
                ...
                <div className="flex -space-x-2">
                  ...
                  {flight.participants.length > 4 && (
                    <div className="h-9 w-9 rounded-full bg-gradient-to-br from-muted to-muted/80 border-2 border-background ring-1 ring-border flex items-center justify-center">
                      <span className="text-xs font-bold text-muted-foreground">+{flight.participants.length - 4}</span>
                    </div>
                  )}
                </div>
                <div className="text-sm text-muted-foreground font-medium">
                  {flight.participants.length} {flight.participants.length === 1 ? "pessoa" : "pessoas"}
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-50 border border-orange-200">
                <AlertTriangle className="w-4 h-4 text-orange-600" />
                <span className="text-sm font-medium text-orange-700">Sem participantes</span>
              </div>
            )}
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex-1 max-w-[140px]">
                <div className="text-center space-y-2">
                  ...
                  <div className="space-y-1">
                    <div className="text-2xl font-bold text-primary tracking-wide">{flight.originAirport}</div>
              ...
              <div className="flex-1 max-w-[140px]">
                <div className="text-center space-y-2">
                  ...
                  <div className="space-y-1">
                    <div className="text-2xl font-bold text-accent tracking-wide">{flight.destinationAirport}</div>
                    <div className="flex items-center justify-center gap-1 text-sm text-muted-foreground">
                      <Calendar className="w-3 h-3" />
            ...
            <div className="flex items-center gap-4">
              {hasCost ? (
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-50 border border-amber-200">
                  ...
                  <span className="text-sm font-medium text-amber-700">Preço não informado</span>
                </div>
              )}
            </div>
