...
function TripLayout() {
  ...
  useEffect(() => {
    ...
    if (!active) return
    const desiredLeft = active.offsetLeft - (el.clientWidth - active.clientWidth) / 2
          ...
          <div className="pt-4 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="space-y-2">
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{travel?.name || "Carregando..."}</h1>
                {travel && (
                  <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                    ...
                    {travel.startDate && travel.endDate && (
                      <div className="flex items-center gap-2">
                        ...
                        <span>{formatTravelDates(travel.startDate, travel.endDate)}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
            ...
            <div className="border-b relative">
              <nav ref={navRef} className="flex overflow-x-auto scrollbar-hide -mb-px">
                {visibleTabs.map((tab) => {
                  const Icon = tab.icon
