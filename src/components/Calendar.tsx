...
export default function Calendar(props: CalendarProps) {
  ...
  return (
    <>
      ...
      <EventDetailsPanel event={selectedEvent} onClose={handleClosePanel} isOpen={isPanelOpen} />
    </>
  )
}
...
const RenderWeekViews = (props: {
  ...
  const accommodationsLayout = props.accommodations
    ...
    : []
  const maxRows =
    accommodationsLayout.length > 0 ? accommodationsLayout.reduce((max, acc) => Math.max(max, acc.row), 0) + 1 : 0
  const allDaySectionHeight =
    props.accommodations && props.accommodations.length > 0 ? Math.max(48, maxRows * 22 + 12) : 0
  ...
  useLayoutEffect(() => {
    ...
    const todayEvents = props.events.sort((a, b) => a.startDate.getTime() - b.startDate.getTime())
    ...
    requestAnimationFrame(() => {
      if (contentScrollAreaRef.current) {
        const scrollElement = contentScrollAreaRef.current.querySelector("[data-radix-scroll-area-viewport]")
        if (scrollElement) {
  ...
  const getDateTimeFromClick = (dayIndex: number, yPosition: number) => {
    ...
    const minuteFromClick = Math.floor(((yPosition % PX_PER_HOUR) / PX_PER_HOUR) * 60)
  ...
  const createEventMutation = useMutation(orpc.eventRoutes.createEvent.mutationOptions())
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 top-0 bottom-0 w-20 border-r z-20 bg-card">
        <div className="absolute left-0 right-0 bottom-0" style={{ top: `${totalHeaderHeight}px` }}>
          <ScrollArea ref={timeRulerRef} className="h-full pointer-events-none" enableThumb={false}>
            <div>
              {TIME_SLOTS.map((time) => (
                <div
                  key={time}
      ...
      <div className="absolute top-0 left-20 right-0 h-16 bg-card border-b z-10">
        <ScrollArea ref={headerScrollAreaRef} className="h-full" enableThumb={false}>
                  ...
                  >
                    ...
                    <div
                      className={`text-lg font-medium ${
                        isToday ? "text-primary font-bold" : isWeekend ? "text-foreground" : "text-foreground"
                      }`}
                    >
                      {date.getDate()}
                    </div>
      ...
      <div className="absolute left-20 right-0 bottom-0" style={{ top: `${totalHeaderHeight}px` }}>
        <ScrollArea ref={contentScrollAreaRef} className="h-full w-full content-scroll-area">
          <div style={{ width: `${totalDays * dayWidth}px` }}>
            <div
              className="grid"
              style={{
...
interface EventBlockProps {
  ...
  eventHeight: number
  isDragging: boolean
  onMouseDown: (event: AppEvent, dayIndex: number, e: React.MouseEvent, resizeHandle?: "top" | "bottom") => void
  onEventClick?: (event: AppEvent) => void
  onUpdateEvent?: (eventId: string, updatedEvent: Partial<AppEvent>) => void
  dayIndex: number
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-start pointer-events-none">
        ...
        {timeDisplay && <div className="text-xs opacity-75 leading-tight mt-0.5">{timeDisplay}</div>}
      </div>
...
interface DayCellProps {
  ...
  onEventMouseDown: (event: AppEvent, dayIndex: number, e: React.MouseEvent, resizeHandle?: "top" | "bottom") => void
  onEventClick?: (event: AppEvent) => void
  onUpdateEvent?: (eventId: string, updatedEvent: Partial<AppEvent>) => void
}
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        {(() => {
          ...
          return dayEvents.map((event) => {
            ...
            const displayStart = eventStart < currentDayStart ? currentDayStart : eventStart
            ...
            const timeDisplay =
              showTime && startTimeStr && endTimeStr && startTimeStr !== endTimeStr
...
const AllDayEventsSection = (props: {
  ...
  const { accommodations, weekDays, dayWidth, allDayScrollAreaRef, sectionHeight } = props

  // Get accommodations for the visible days and layout them to avoid overlaps
  const accommodationsLayout = getAccommodationsLayoutWithRows(accommodations, weekDays)

  return (
    <div className="absolute top-16 left-20 right-0 bg-muted/20 border-b z-15" style={{ height: `${sectionHeight}px` }}>
      <ScrollArea ref={allDayScrollAreaRef} className="h-full" enableThumb={false}>
        <div style={{ width: `${weekDays.length * dayWidth}px` }}>
          <div className="relative p-1" style={{ height: `${sectionHeight - 8}px` }}>
...
const getAccommodationsLayoutWithRows = (
  ...
  // Sort by start date
  validAccommodations.sort((a, b) => a.startDate.getTime() - b.startDate.getTime())
