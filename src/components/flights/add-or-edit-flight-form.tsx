...
const flightFormSchema = z
  .object({
    ...
    participantIds: z.array(z.string()).min(0, "Selecione pelo menos um participante"),
  })
...
export function AddOrEditFlightForm({
  ...
  const form = useForm<FlightFormData>({
    ...
    defaultValues: {
      ...
      departureDate: flight ? new Date(flight.departureDate).toISOString().split("T")[0] : "",
      departureTime: flight?.departureTime || "",
      arrivalDate: flight ? new Date(flight.arrivalDate).toISOString().split("T")[0] : "",
  ...
  // Mutations
  const createFlightMutation = useMutation(orpc.flightRoutes.createFlight.mutationOptions())
  const updateFlightMutation = useMutation(orpc.flightRoutes.updateFlight.mutationOptions())
  const addParticipantMutation = useMutation(orpc.flightRoutes.addFlightParticipant.mutationOptions())
  const removeParticipantMutation = useMutation(orpc.flightRoutes.removeFlightParticipant.mutationOptions())
  ...
  const onSubmit = async (data: FlightFormData) => {
    try {
      if (isEditMode && flight) {
        ...
        const participantsToAdd = data.participantIds.filter((id) => !currentParticipantIds.includes(id))
        const participantsToRemove = currentParticipantIds.filter((id) => !data.participantIds.includes(id))
  ...
  const isSubmitting = isEditMode ? updateFlightMutation.isPending : createFlightMutation.isPending

  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FlightDetailsSection form={form} setDuplicateInfo={isEditMode ? () => {} : setDuplicateInfo} />

        <FlightTimesSection form={form} minDate={minDate} maxDate={maxDate} />

        <FlightCostSection form={form} setDuplicateInfo={isEditMode ? () => {} : setDuplicateInfo} />

        {!isEditMode && duplicateInfo?.isDuplicate && (
          <Alert>
            <Info className="h-4 w-4" />
            <AlertDescription>
              {duplicateInfo.message} Os participantes selecionados serão adicionados ao voo existente.
            </AlertDescription>
          </Alert>
        )}
...
function FlightParticipantsSection({ form, members }: { form: UseFormReturn<FlightFormData>; members: Member[] }) {
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel className="font-semibold text-base text-foreground">Participantes</FormLabel>
          <FormControl>
              ...
              {members.map((member) => (
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        ...
                        field.onChange(field.value.filter((id: string) => id !== member.id))
                      }
                    }}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Label htmlFor={member.id} className="font-medium cursor-pointer flex-1">
                      {member.name}
                    </Label>
                  </div>
                </div>
...
function FlightDetailsSection({
  ...
  const [recommendedOriginOptions, setRecommendedOriginOptions] = useState<LocationOption[]>([])
  const [recommendedDestinationOptions, setRecommendedDestinationOptions] = useState<LocationOption[]>([])
  ...
  const { data: destinationResults = [] } = useAirportsSearch(destinationSearch, 10)
  ...
  return (
    <div className="space-y-6">
      <h4 className="font-semibold text-base text-foreground">Detalhes do Voo</h4>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-6">
          <FormField
            ...
            name="originAirport"
            render={({ field }) => {
              const selectedOrigin = field.value ? [{ value: field.value, label: field.value }] : []
          ...
          <FormField
            ...
            render={({ field }) => {
              const selectedDestination = field.value ? [{ value: field.value, label: field.value }] : []
...
function FlightTimesSection({
  ...
  const arrivalTimeMin =
    arrivalDate && departureDate && arrivalDate === departureDate ? departureTime || undefined : undefined
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} type="date" min={minDate} max={maxDate} className="h-11" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Data *</FormLabel>
                <FormControl>
                  <Input {...field} type="date" min={arrivalMinDate} max={maxDate} className="h-11" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Horário *</FormLabel>
                <FormControl>
                  <Input {...field} type="time" min={arrivalTimeMin} className="h-11" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
