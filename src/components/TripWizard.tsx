...
export default function TripWizard({ initialData }: TripWizardProps) {
	...
	const handleFinish = form.handleSubmit(async (data) => {
		...
		const travelData: InsertFullTravel = {
			...
			locationInfo: {
				destination: data.destinations?.[0]?.label || "Destino não especificado",
				country: "Brazil", // Default, should be inferred from destination
				climate: "Tropical", // Default
				currency: "BRL", // Default
				language: "Portuguese", // Default
				timeZone: "America/Sao_Paulo", // Default
				bestTimeToVisit: "Durante todo o ano", // Default
			},
			visaInfo: {
				required: false, // Default
				stayDuration: "90 dias", // Default
				documents: ["Passaporte"], // Default
