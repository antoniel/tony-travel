...
import { CalendarIcon, Clock, DollarSign, MapPin, Plane, Users, X } from "lucide-react"
            ...
            <Card className="travel-card rounded-2xl ring-1 ring-border/60">
              <CardContent className="px-8 py-4 space-y-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        ...
                        {form.people === "custom" ? (
                          ...
                          <Select
                            value={form.people}
                            onValueChange={(value) => setForm((prev) => ({ ...prev, people: value }))}
                          >
                  ...
                  <div className="space-y-3">
                    <Label className="text-base font-medium">Quando você quer viajar?</Label>
                    <Popover open={isDatePopoverOpen} onOpenChange={setIsDatePopoverOpen}>
                      ...
                      <PopoverContent className="w-auto p-0" align="center">
                        <Calendar
                          ...
                          onSelect={(dateRange) => setForm((prev) => ({ ...prev, dateRange }))}
                          numberOfMonths={2}
                          locale={ptBR}
                ...
                <div className="pt-4">
                  <Button
                    onClick={() => {
                      ...
                      try {
                        localStorage.setItem("tt_last_departure_airports", JSON.stringify(form.departureAirports))
                        localStorage.setItem("tt_last_destinations", JSON.stringify(form.destinations))
                      } catch {}
                      ...
                      const searchParams = {
                        ...
                        departureAirports:
                          form.departureAirports.length > 0 ? JSON.stringify(form.departureAirports) : undefined,
                      }
                  ...
                  >
                    <span className="relative z-10">Criar meu roteiro personalizado</span>
                  </Button>
                </div>
              </CardContent>
            </Card>
          ...
          <Dialog open={loginModalOpen} onOpenChange={setLoginModalOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>Você precisa estar logado para criar um roteiro personalizado.</DialogDescription>
              </DialogHeader>
              <div className="flex flex-col gap-4 py-4">
                <Button onClick={handleGoogleLogin} className="w-full h-12 gap-3" variant="outline">
                ...
                >
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-lg mb-2">{trip.name}</CardTitle>
                        <div className="flex items-center text-muted-foreground mb-2">
                          <MapPin className="h-4 w-4 mr-1" />
                          <span className="text-sm">{trip.destination}</span>
                        </div>
                      </div>
                      <Badge variant="secondary">{getDurationInDays(trip.startDate, trip.endDate)} dias</Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center text-sm text-muted-foreground">
                        ...
                        <span>A partir de R$ {formatNumberPtBR(getSelectedBudget())}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
...
function DialogCreateTravel(props: {
  ...
  function tryImportTravel() {
    ...
    try {
      ...
      setImportError("JSON inválido. Verifique vírgulas, aspas e datas como strings ISO.")
      return
    }
    ...
    if (!t || !t.name || !t.startDate || !t.endDate || !t.events) {
      setImportError("Objeto inválido: faltam campos obrigatórios (name, dates, events).")
      return
    }
    ...
    <Dialog open={props.promptOpen} onOpenChange={props.setPromptOpen}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            1) Copie o prompt abaixo e cole no ChatGPT. 2) Cole aqui APENAS um bloco <code>```json</code> com o objeto
            Travel (nenhum texto fora do bloco).
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            ...
            <div className="mt-2 flex gap-2">
              <Button size="sm" onClick={() => props.copyToClipboard(props.generatedPrompt)}>
                Copiar
              </Button>
            </div>
          </div>
