...
import { ALWAYS_USER_TEST, createAppCall, createAppCallAuthenticated, getFakeDb, testStub } from "@/tests/utils"
...
describe("travel service", () => {
  ...
  describe("soft delete behavior", () => {
    it("should exclude soft deleted travels from getAllTravels", async () => {
      ...
      const travelStub1 = testStub.travel()
      const [travel1] = await db.insert(Travel).values(travelStub1).returning({ id: Travel.id })
