...
export class TravelDAO {
  ...
  async createTravel(travelData: TravelInput & { userId: string }): Promise<string> {
    ...
    if (travelData.accommodations.length > 0) {
      await this.db.insert(Accommodation).values(travelData.accommodations.map((a) => ({ ...a, travelId })))
    }
  ...
  async createTravelWithTransaction(
    ...
    if (travelData.accommodations.length > 0) {
      await tx.insert(Accommodation).values(travelData.accommodations.map((a) => ({ ...a, travelId })))
    }
  ...
  async createTravelMember(memberData: z.infer<typeof InsertTravelMemberSchema>): Promise<string> {
    const [member] = await this.db.insert(TravelMember).values(memberData).returning({ id: TravelMember.id })
    return member.id
  }
  ...
  async createTravelMemberWithTransaction(
    ...
    const [member] = await tx.insert(TravelMember).values(memberData).returning({ id: TravelMember.id })
    return member.id
  }

  async getTravelMember(travelId: string, userId: string): Promise<typeof TravelMember.$inferSelect | null> {
    const member = await this.db.query.TravelMember.findFirst({
      where: and(eq(TravelMember.travelId, travelId), eq(TravelMember.userId, userId)),
      with: {
        user: true,
      },
    })
  ...
  async getTravelMembers(travelId: string): Promise<(typeof TravelMember.$inferSelect)[]> {
  ...
  async deleteTravelMember(travelId: string, userId: string): Promise<void> {
    await this.db.delete(TravelMember).where(and(eq(TravelMember.travelId, travelId), eq(TravelMember.userId, userId)))
  }

  async updateTravelMemberRole(travelId: string, userId: string, newRole: "owner" | "member"): Promise<void> {
    await this.db
      .update(TravelMember)
      .set({ role: newRole })
      .where(and(eq(TravelMember.travelId, travelId), eq(TravelMember.userId, userId)))
  ...
  async softDeleteTravel(id: string, deletedBy: string): Promise<typeof Travel.$inferSelect | null> {
  ...
  async getTravelByIdIncludingDeleted(id: string): Promise<typeof Travel.$inferSelect | null> {
    const travel = await this.db.query.Travel.findFirst({
