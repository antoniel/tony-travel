...
import { authProcedure, optionalAuthProcedure, travelMemberProcedure } from "@/orpc/procedure"
...
export const saveTravel = authProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    const result = await createTravelService(context.db, travelDAO, context.user.id, input.travel)
...
export const getTravel = optionalAuthProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    const result = await getTravelService(travelDAO, input.id, context.user?.id)
...
export const searchAirports = os
  ...
  .handler(({ input }) => {
    ...
    let filteredAirports: Airport[] = enhancedAirports.map((airport) => ({
      ...
      type: airport.type as "airport" | "city_group" | "state_group" | "country_group",
    ...
    if (query.trim()) {
      ...
      filteredAirports = filteredAirports.filter((airport) => {
        ...
        const matchesState = airport.state?.toLowerCase().includes(searchTerm) || false
        const matchesCountry = airport.country.toLowerCase().includes(searchTerm)
        const matchesIata = airport.code.toLowerCase().includes(searchTerm)
        const matchesCountryCode = airport.countryCode.toLowerCase().includes(searchTerm)

        return matchesName || matchesCity || matchesState || matchesCountry || matchesIata || matchesCountryCode
      })
    }
    ...
    return filteredAirports.slice(0, limit).sort((a, b) => {
      ...
      if (a.type === "state_group" && b.type !== "state_group" && b.type !== "country_group") return -1
      if (a.type !== "state_group" && a.type !== "country_group" && b.type === "state_group") return 1
...
export const searchDestinations = os
  ...
  .handler(({ input }) => {
    ...
    return filteredDestinations.slice(0, limit).sort((a, b) => a.label.localeCompare(b.label))
  })
...
export const updateTravel = authProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    const result = await updateTravelService(travelDAO, input.travelId, context.user.id, input.updateData)
...
export const deleteTravel = authProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    const result = await softDeleteTravelService(travelDAO, input.travelId, context.user.id, input.confirmationName)
