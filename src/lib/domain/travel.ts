 ...
 */
export function calculateEventTotalCost(event: AppEvent, dependencies: AppEvent[] | undefined): number {
  const baseCost =
 ...
 */
export function calculateTravelTotalCost(travel: TravelWithRelations | undefined): number {
  if (!travel) return 0

  return travel.events.reduce((acc, event) => acc + calculateEventTotalCost(event, event.dependencies), 0)
}

/**
 * Calculate the number of days in a travel
 */
export function calculateTravelDays(travel: TravelWithRelations | undefined): number {
  if (!travel) return 0
