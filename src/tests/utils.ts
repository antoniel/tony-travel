...
import { type ErrorMap, type Lazyable, type Meta, type Procedure, call } from "@orpc/server"
...
export const createAppCall =
  (db: DB) =>
  <TInputSchema extends AnySchema, TOutputSchema extends AnySchema, TErrorMap extends ErrorMap, TMeta extends Meta>(
    procedure: Lazyable<
      Procedure<
        AppOrpcContext,
    ...
    return call<AppOrpcContext, TInputSchema, TOutputSchema, TErrorMap, TMeta>(procedure, input, {
      context: {
        db: db as DB,
      },
    })
...
export const testStub = {
  ...
  flightParticipant: (flightParticipant?: Partial<schema.FlightParticipant>) => {
